/**
 * @fileOverview kissy-portal 组件的安全适配器
...
KISSY.add(function (S, Gallery) {
    ...
    function init(frameGroup) {
         ...
         */
        function SafeKissyPortal(el, config) {
            this.inner = new KissyPortal(el, config);//内部实际封装了真正的组件构造函数
        }

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法，on和destory只是举例，哪些需要暴露的方法就写哪些就行了
        SafeKissyPortal.prototype.on = function (type,fnc) {
            //this.inner.on(type,fnc);
        };

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法
        SafeKissyPortal.prototype.destory = function () {
            //this.inner.destory();
        };
        ...
        frameGroup.markCtor(SafeKissyPortal);

        frameGroup.grantMethod(SafeKissyPortal, "on");
        frameGroup.grantMethod(SafeKissyPortal, "destory");
        ...
        return function (context) {
            ...
            //最终需要返回给
            return {
                //这里的 kissy-portal 会自动放到KISSY.KissyPortal:
            KissyPortal: frameGroup.markFunction(function () {
                    return new SafeKissyPortal(arguments[0], cajaAFTB.untame(arguments[1]));
                }),
                kissy:true
            }
        }
